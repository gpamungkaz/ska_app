...
/// Global helper to get position with error toast
/// Set shortTimeout=true for testing (2 sec) or false for production (10 sec)
Future<Position?> getPositionWithErrorToast(
  ...
  try {
    final position = await Geolocator.getCurrentPosition(
      ...
      timeLimit: Duration(seconds: shortTimeout ? 2 : 10),
    );
    return position;
  } catch (e) {
    if (kDebugMode) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: const Text(
          '📍 Lokasi tidak tersedia. Pastikan GPS aktif dan sinyal kuat.',
        ),
...
class _MarketingHomeScreenState extends State<MarketingHomeScreen> {
  ...
  final TextEditingController _unitMovementSearchController =
  ...
  Future<void> _startNonVisitFlow() async {
    final payload = await showModalBottomSheet<VisitSubmissionPayload>(
      context: context,
      useSafeArea: true,
      isScrollControlled: true,
      ...
      builder: (context) => _NonVisitSheet(
        authToken: widget.authToken,
        onCompleted: (result) => Navigator.of(context).pop(result),
      ),
    );
  ...
  Future<void> _submitVisit(VisitSubmissionPayload payload) async {
    ...
    try {
      final request = http.MultipartRequest('POST', Uri.parse(_visitsEndpoint))
        ..headers.addAll({
          'Accept': 'application/json',
          'Authorization': 'Bearer ${widget.authToken}',
        });
      ...
      final fields = <String, String>{
        'visit_type': payload.type == VisitDealerType.existing
            ? 'existing'
            : payload.type == VisitDealerType.newDealer
            ? 'new'
            : 'non_visit',
      ...
      } else if (payload.type == VisitDealerType.newDealer) {
      ...
      } else if (payload.type == VisitDealerType.nonVisit) {
        fields['agenda_id'] = payload.agendaId?.toString() ?? '';
        fields['inviting_name'] = payload.invitingName ?? '';
        fields['agenda_description'] = payload.agendaDescription ?? '';
      }
  ...
  Future<void> _fetchUnitMovementOrders() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
          '${ApiConfig.baseUrl}/api/v1/purchase-orders?status[]=approved&status[]=in_progress',
        ),
        headers: {
          'Accept': 'application/json',
          'Authorization': 'Bearer ${widget.authToken}',
  ...
  Future<void> _submitPurchaseOrder(PurchaseOrderPayload payload) async {
    ...
    try {
      final body = jsonEncode({
        ...
        'inner_length': payload.innerLength,
      ...
      if (response.statusCode >= 200 &&
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('SPK berhasil disimpan.')));
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      _buildMetaRow(
                                        ...
                                        label:
                                            'Dibuat pada ${order.createdAtLabel}',
                                      ),
                                    ],
                                  ),
                                ),
      ...
      children: [
        Row(
          ...
          children: [
            ...
            OutlinedButton(
              onPressed: _isUnitMovementLoading
                  ? null
                  : _fetchUnitMovementOrders,
              style: OutlinedButton.styleFrom(
        ...
        TextField(
          ...
          decoration: InputDecoration(
            hintText:
                'Cari SPK berdasarkan kode SPK, customer, dealer, merk, model, atau nomor rangka...',
...
class OwnerHomeScreen extends StatefulWidget {
  const OwnerHomeScreen({super.key, this.userName, required this.authToken});
...
class _OwnerHomeScreenState extends State<OwnerHomeScreen> {
  ...
  String _visitFilterType =
      'week'; // 'today', 'week', 'month', 'year', 'custom'
    ...
    try {
      ...
      if (filterType != null) {
        if (filterType == 'custom' && customRange != null) {
          queryParams['start_date'] = customRange.start.toIso8601String().split(
            'T',
          )[0];
          queryParams['end_date'] = customRange.end.toIso8601String().split(
            'T',
          )[0];
        } else {
          switch (filterType) {
  ...
  Future<void> _fetchPurchaseOrders({
    String? filterType,
    DateTimeRange? customRange,
  }) async {
    if (!mounted) return;
...
class _VisitCreationOptions extends StatelessWidget {
  const _VisitCreationOptions({
    ...
    required this.onSelectNonVisit,
  });

  final VoidCallback onSelectExisting;
  final VoidCallback onSelectNew;
  final VoidCallback onSelectNonVisit;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(
            'Pilih tipe kunjungan atau agenda.',
            style:
          ...
          _VisitCreationOptionTile(
            ...
            title: 'Agenda Non Kunjungan',
            subtitle: 'Catat kegiatan selain kunjungan dealer.',
            onTap: onSelectNonVisit,
          ),
        ],
      ),
    );
...
class _DealerErrorState extends StatelessWidget {
  const _DealerErrorState({required this.message, required this.onRetry});
...
class _UnitMovementErrorState extends StatelessWidget {
  const _UnitMovementErrorState({required this.message, required this.onRetry});
...
class _NewDealerVisitSheet extends StatefulWidget {
  const _NewDealerVisitSheet({required this.onCompleted});
...
class _ExistingDealerVisitSheet extends StatefulWidget {
  ...
  @override
  State<_ExistingDealerVisitSheet> createState() =>
      _ExistingDealerVisitSheetState();
}
...
class _ExistingDealerVisitSheetState extends State<_ExistingDealerVisitSheet> {
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      // Use helper with shortTimeout=true for easy testing in browser
      final position = await getPositionWithErrorToast(
        context,
        shortTimeout: true, // Change to false for production
      );
      ...
      if (selfie == null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Selfie dibatalkan. Kunjungan belum disimpan.'),
          ),
        );
        return;
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Gagal membuka kamera: ${error.message ?? error.code}'),
        ),
      );
    } catch (error) {
      if (!mounted) return;
  ...
  Future<bool> _ensureLocationPermission() async {
    ...
    if (permission == LocationPermission.denied ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Izin lokasi diperlukan untuk mencatat koordinat kunjungan.',
          ),
        ),
      );
      return false;
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                  _isProcessing ? 'Memproses...' : 'Simpan & Ambil Selfie',
                ),
  ...
  Future<bool> _ensureLocationPermission() async {
    ...
    if (permission == LocationPermission.denied ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Izin lokasi diperlukan untuk mencatat koordinat visit.',
          ),
        ),
      );
      return false;
...
class _NonVisitSheet extends StatefulWidget {
  const _NonVisitSheet({required this.authToken, required this.onCompleted});

  final String authToken;
  final ValueChanged<VisitSubmissionPayload> onCompleted;

  @override
  State<_NonVisitSheet> createState() => _NonVisitSheetState();
}

class _NonVisitSheetState extends State<_NonVisitSheet> {
  final _formKey = GlobalKey<FormState>();
  final TextEditingController _invitingNameController = TextEditingController();
  final TextEditingController _agendaDescriptionController =
      TextEditingController();
  final ImagePicker _picker = ImagePicker();

  List<AgendaData> _agendas = const [];
  AgendaData? _selectedAgenda;
  bool _isLoadingAgendas = true;
  bool _isProcessing = false;
  String? _agendasErrorMessage;
  ...
  @override
  void dispose() {
    _invitingNameController.dispose();
  ...
  Future<void> _loadAgendas() async {
    if (kDebugMode) {
      print('🔧 [_loadAgendas] Loading agendas from API...');
    }

    setState(() {
      _isLoadingAgendas = true;
      _agendasErrorMessage = null;
    });

    try {
      final url = '${ApiConfig.baseUrl}/api/v1/agendas';
      if (kDebugMode) {
        print('🔧 [_loadAgendas] URL: $url');
      }
      ...
      if (kDebugMode) {
        print('🔧 [_loadAgendas] Status Code: ${response.statusCode}');
        print('🔧 [_loadAgendas] Response Body: ${response.body}');
      }

      if (response.statusCode == 401) {
        if (!mounted) return;
        setState(() {
          _agendasErrorMessage = 'Sesi login berakhir. Silakan login kembali.';
          _isLoadingAgendas = false;
        });
        return;
      }
      ...
      if (response.statusCode >= 200 &&
        ...
        final raw = decoded['data'];
        if (kDebugMode) {
          print('🔧 [_loadAgendas] Raw data type: ${raw.runtimeType}');
          print('🔧 [_loadAgendas] Raw data: $raw');
        }
        ...
        if (raw is List) {
          ...
          if (innerData is List) {
            agendasList = innerData;
          }
        }

        final agendas = agendasList
            .whereType<Map<String, dynamic>>()
            .map(AgendaData.fromJson)
            .toList();

        if (kDebugMode) {
          print('🔧 [_loadAgendas] Parsed agendas count: ${agendas.length}');
          for (var agenda in agendas) {
            print(
              '🔧 [_loadAgendas] Agenda: id=${agenda.id}, name=${agenda.name}',
            );
          }
        }
        ...
        final message =
            ...
            'Gagal memuat daftar agenda. Status: ${response.statusCode}';
        throw Exception(message);
      }
    } catch (error) {
      if (kDebugMode) {
        print('🔧 [_loadAgendas] Error: $error');
      }
      if (!mounted) return;
      setState(() {
        _agendasErrorMessage = 'Terjadi kesalahan: ${error.toString()}';
        _isLoadingAgendas = false;
      });
    }
  }
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_selectedAgenda == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Pilih jenis agenda terlebih dahulu')),
      );
      return;
    }
    ...
    try {
      ...
      if (selfie == null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Selfie dibatalkan. Agenda belum disimpan.'),
          ),
        );
        return;
      }
      ...
      final payload = VisitSubmissionPayload(
        type: VisitDealerType.nonVisit,
        agendaId: _selectedAgenda!.id,
        invitingName: _invitingNameController.text.trim(),
        agendaDescription: _agendaDescriptionController.text.trim(),
        selfie: selfie,
        latitude: position.latitude,
        longitude: position.longitude,
      );
  ...
  Future<bool> _ensureLocationPermission() async {
    ...
    if (permission == LocationPermission.denied ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Izin lokasi diperlukan untuk mencatat agenda.'),
        ),
      );
      return false;
    }
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Catat kegiatan selain kunjungan dealer',
              ...
              Builder(
                builder: (context) {
                  if (kDebugMode) {
                    print(
                      '🔧 [_NonVisitSheet.build] _isLoadingAgendas: $_isLoadingAgendas',
                    );
                    print(
                      '🔧 [_NonVisitSheet.build] _agendasErrorMessage: $_agendasErrorMessage',
                    );
                    print(
                      '🔧 [_NonVisitSheet.build] _agendas.length: ${_agendas.length}',
                    );
                  }

                  if (_isLoadingAgendas) {
                    ...
                    return Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          const Expanded(
                            child: Text(
                              'Belum ada data agenda tersedia.',
                  ...
                  return DropdownButtonFormField<AgendaData>(
                    value: _selectedAgenda,
                    decoration: InputDecoration(
                      labelText: 'Jenis Agenda *',
                      hintText: 'Pilih jenis agenda',
              ...
              const SizedBox(height: 16), // Nama Pengundang
              TextFormField(
                controller: _invitingNameController,
                decoration: InputDecoration(
                  labelText: 'Nama Pengundang *',
                  hintText: 'Masukkan nama pengundang',
                ...
                validator: (value) {
                  if (value == null || value.trim().isEmpty) {
                    return 'Nama pengundang harus diisi';
                  }
                  return null;
                },
              ),
              ...
              TextFormField(
                ...
                validator: (value) {
                  if (value == null || value.trim().isEmpty) {
                    return 'Keterangan kegiatan harus diisi';
                  }
                  return null;
                },
              ),
      ...
      child: Column(
        ...
        children: [
          ...
          const _InstructionBullet(
            text: 'Tekan tombol mulai untuk memulai proses.',
          ),
          const _InstructionBullet(
            text: 'Pastikan wajah Anda berada di dalam bingkai.',
          ),
          const _InstructionBullet(
...
enum VisitDealerType { existing, newDealer, nonVisit }

class AgendaData {
  const AgendaData({required this.id, required this.name});

  final int id;
  final String name;

  factory AgendaData.fromJson(Map<String, dynamic> json) {
    // Parse ID dengan berbagai kemungkinan tipe
    int parsedId = 0;
    final idValue = json['id'] ?? json['agenda_id'];
    if (idValue is int) {
      parsedId = idValue;
    ...
    // Parse name dengan berbagai kemungkinan field name
    String parsedName = 'Agenda';
    final nameValue =
        json['name'] ??
        json['agenda_name'] ??
        json['title'] ??
    ...
    if (nameValue != null && nameValue.toString().trim().isNotEmpty) {
      parsedName = nameValue.toString().trim();
    }

    return AgendaData(id: parsedId, name: parsedName);
  }
}
...
class VisitSubmissionPayload {
  ...
  // Non-visit specific fields
  final int? agendaId;
  final String? invitingName;
...
class _VisitDetailSheet extends StatelessWidget {
  ...
  static Future<void> show(
    ...
    return showModalBottomSheet<void>(
      ...
      builder: (context) =>
          _VisitDetailSheet(visit: visit, authToken: authToken),
    );
  }
  ...
  Future<void> _openGoogleMaps(BuildContext context) async {
    ...
    if (latitude == null || longitude == null) {
      messenger
        ?..hideCurrentSnackBar()
        ..showSnackBar(
          const SnackBar(content: Text('Koordinat kunjungan tidak tersedia.')),
        );
      return;
    }
  ...
  Future<void> _loadImage() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode) {
          print(
            '🔧 Image loaded successfully, size: ${response.bodyBytes.length} bytes',
          );
        }
      } else {
        setState(() {
...
class _VisitPlaceholderCard extends StatelessWidget {
  const _VisitPlaceholderCard({required this.icon, required this.message});
                ...
                MarkerLayer(
                  markers: [
                    Marker(
                      ...
                      child: _VisitMapMarker(label: coordinateLabel),
                    ),
                  ],
                ),
...
class _OwnerVisitView extends StatefulWidget {
  ...
  final void Function(String filterType, DateTimeRange? dateRange)
  onFilterChanged;
...
class _AddPurchaseOrderSheetState extends State<_AddPurchaseOrderSheet> {
  ...
  void _handleSubmit() {
    ...
    final payload = PurchaseOrderPayload(
      ...
      innerLength: _innerLengthController.text.trim().isEmpty
          ? null
          : _innerLengthController.text.trim(),
      innerWidth: _innerWidthController.text.trim().isEmpty
          ? null
          : _innerWidthController.text.trim(),
      innerHeight: _innerHeightController.text.trim().isEmpty
          ? null
          : _innerHeightController.text.trim(),
      optional: _optionalController.text.trim().isEmpty
          ? null
            ...
            children: [
                ...
                fieldViewBuilder:
                    (context, controller, focusNode, onFieldSubmitted) {
                      ...
                      return TextFormField(
                        ...
                        decoration: InputDecoration(
                          labelText: 'Dealer',
                          hintText: _isLoadingDealers
                              ? 'Memuat dealer...'
                              : 'Cari atau pilih dealer',
              ...
              Autocomplete<BodyTypeData>(
                optionsBuilder: (textEditingValue) {
                  ...
                  return _bodyTypes.where((type) {
                    return type.name.toLowerCase().contains(
                      textEditingValue.text.toLowerCase(),
                    );
                  });
                },
                    ...
                    (context, controller, focusNode, onFieldSubmitted) {
                      ...
                      return TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintText: _isLoadingBodyTypes
                              ? 'Memuat tipe pengerjaan...'
                              : 'Cari atau pilih pengerjaan',
                        ...
                        validator: (value) => _selectedBodyType == null
                            ? 'Pilih tipe pengerjaan'
                            : null,
                      );
                    },
                optionsViewBuilder: (context, onSelected, options) {
              ...
              Row(
                children: [
                  Expanded(
                    child: TextFormField(
                      controller: _innerLengthController,
                      decoration: InputDecoration(
                        labelText: 'Panjang Dalam',
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(12),
                        ),
                      ),
                  ...
                  Expanded(
                    child: TextFormField(
                      controller: _innerHeightController,
                      decoration: InputDecoration(
                        labelText: 'Tinggi Dalam',
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(12),
                        ),
                      ),
              ...
              const SizedBox(height: 12),
              TextFormField(
                controller: _innerWidthController,
                decoration: InputDecoration(
                  labelText: 'Lebar Dalam',
                  border: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(12),
                  ),
                ),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      'Rp ${_totalPrice.toStringAsFixed(0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (match) => '${match[1]}.')}',
                      style: const TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.bold,
                        color: Colors.orange,
...
class VisitData {
  ...
  factory VisitData.fromJson(
    ...
    final selfieLabel = _clean(
      _readString(selfieMap, ['label', 'title', 'name', 'description']) ??
          _readString(json, [
            'selfie_label',
            'photo_label',
            'documentation_label',
    ...
    String? fixSelfieUrl(String? url) {
      ...
      if ((path.startsWith('/visits/') || path.startsWith('visits/')) &&
          !path.contains('/storage/')) {
        final fixed = url.replaceFirst(
          RegExp(r'/?visits/'),
          '/storage/visits/',
        );
        if (kDebugMode) {
  ...
  static String? _resolveUrl(String? value, {String? baseUrl}) {
    ...
    try {
      ...
      if (trimmed.startsWith('visits/') && !trimmed.contains('/storage/')) {
        ...
        if (kDebugMode) {
          print(
            '🔧 [_resolveUrl] Fixed path (fallback): $trimmed -> $pathToUse',
          );
        }
      }
...
class BodyTypeData {
  const BodyTypeData({required this.id, required this.name});
...
class PurchaseOrderData {
  ...
  String get userName =>
      user?['name']?.toString() ?? 'Marketing Tidak Diketahui';

  String get formattedTotalPrice {
    final formatter = totalPrice
        .toStringAsFixed(0)
        .replaceAllMapped(
  ...
  factory PurchaseOrderData.fromJson(Map<String, dynamic> json) {
    ...
    return PurchaseOrderData(
      ...
      code:
          ...
          json['spk_number']?.toString().trim() ??
          json['spk_code']?.toString().trim(),
      user: json['user'] as Map<String, dynamic>?,
      dealerName:
          dealerData?['name']?.toString().trim() ??
      ...
      bodyTypeName:
          bodyTypeData?['name']?.toString().trim() ??
      ...
      innerLength: json['inner_length']?.toString().trim(),
      innerWidth: json['inner_width']?.toString().trim(),
      innerHeight: json['inner_height']?.toString().trim(),
      ...
      progressAt: (() {
        final raw =
            json['progressed_at'] ?? json['in_at'] ?? json['progressAt'];
      ...
      completedAt: (() {
        final raw =
            json['completed_at'] ?? json['out_at'] ?? json['completedAt'];
        if (raw == null) return null;
        if (raw is String) return DateTime.tryParse(raw);
        if (raw is int) return DateTime.fromMillisecondsSinceEpoch(raw * 1000);
        return null;
...
class _SpkDetailSheet extends StatelessWidget {
  const _SpkDetailSheet({required this.spk, required this.authToken});
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (spk.innerLength != null ||
                            spk.innerWidth != null ||
                            spk.innerHeight != null) ...[
                          const SizedBox(height: 28),
                          Text(
                            'Dimensi Dalam',
                            style:
                          ...
                          const SizedBox(height: 16),
                          if (spk.innerLength != null &&
                              spk.innerLength!.isNotEmpty)
                            _VisitDetailItem(
                              icon: Icons.straighten_outlined,
                              title: 'Panjang Dalam',
                              value: '${spk.innerLength} cm',
                            ),
                          if (spk.innerHeight != null &&
                              spk.innerHeight!.isNotEmpty)
                            _VisitDetailItem(
                              icon: Icons.height_outlined,
                              title: 'Tinggi Dalam',
                              value: '${spk.innerHeight} cm',
                            ),
                          if (spk.innerWidth != null &&
                              spk.innerWidth!.isNotEmpty)
                            _VisitDetailItem(
                              icon: Icons.swap_horiz_outlined,
                              title: 'Lebar Dalam',
                              value: '${spk.innerWidth} cm',
                            ),
                        ],
                        if (spk.optional != null &&
                            spk.optional!.isNotEmpty) ...[
                          const SizedBox(height: 16),
                          _VisitDetailItem(
                        ...
                        _VisitDetailItem(
                          ...
                          value:
                              'Rp ${spk.unitPrice.toStringAsFixed(0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (match) => '${match[1]}.')}',
                        ),
                        _VisitDetailItem(
                          icon: Icons.inventory_2_outlined,
                          title: 'Jumlah',
...
class _UnitMovementSearchSheet extends StatefulWidget {
  const _UnitMovementSearchSheet({required this.authToken});

  final String authToken;

  @override
  State<_UnitMovementSearchSheet> createState() =>
      _UnitMovementSearchSheetState();
}
...
class _SpkTile extends StatelessWidget {
  const _SpkTile({required this.order, required this.onTap});
...
class _UnitMovementDetailSheet extends StatefulWidget {
  ...
  static Future<void> show(
    ...
    String authToken, {
    VoidCallback? onMovementSuccess,
  }) {
    return showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => _UnitMovementDetailSheet(
        order: order,
        authToken: authToken,
        onMovementSuccess: onMovementSuccess,
      ),
    );
  }

  @override
  State<_UnitMovementDetailSheet> createState() =>
      _UnitMovementDetailSheetState();
}
  ...
  Future<void> _handleMovement(String type) async {
    ...
    try {
      ...
      final response = await http.patch(
        Uri.parse(
          '${ApiConfig.baseUrl}/api/v1/purchase-orders/${widget.order.id}/status',
        ),
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
      ...
      if (response.statusCode >= 200 &&
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Unit berhasil ${type == 'in' ? 'masuk' : 'keluar'}'),
          ),
        );
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (widget.order.optional != null &&
                            widget.order.optional!.isNotEmpty) ...[
                          const SizedBox(height: 16),
                          _VisitDetailItem(
                            icon: Icons.note_add_outlined,
                            title: 'Opsional',
                        ...
                        _VisitDetailItem(
                          ...
                          title: 'Harga per Unit',
                          value:
                              'Rp ${widget.order.unitPrice.toStringAsFixed(0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (match) => '${match[1]}.')}',
                        ),
                        _VisitDetailItem(
                          icon: Icons.inventory_2_outlined,
                          title: 'Jumlah',
